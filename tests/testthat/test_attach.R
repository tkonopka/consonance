...
test_that("test_consonance using a model with consonance", {
  ...
  expect_silent(validate(xy, suite))
  expect_silent(validate(xy, model))
})
...
test_that("test_consonance using a model with consonance suite", {
  ...
  expect_error(capture_output(validate(xy.bad, suite)))
  expect_error(capture_output(validate(xy.bad, model)))
})
...
test_that("attaching sets function environment", {
  ...
  expect_silent(validate(c(12, 14), model))
  expect_error(capture_output(validate(c(5, 15), model)))
})
