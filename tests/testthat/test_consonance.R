...
test_that("consonance checks suite class", {
  expect_error(validate_consonance(1:10, 10), "not a consonance suite")
})
...
test_that("test_consonance with assert / check / test syntax", {
  expect_silent(validate_consonance(abc, suite_assert))
  expect_silent(validate_consonance(abc, suite_check))
  expect_silent(validate_consonance(abc, suite_test))
  expect_error(capture_output(validate_consonance(1:3, suite_verbose)))
  expect_error(capture_output(validate_consonance(1:3, suite_check)))
  expect_error(capture_output(validate_consonance(1:3, suite_test)))
})

test_that("test_consonance with a single test", {
  expect_silent(validate_consonance(abc, single_test))
  expect_silent(validate_consonance(abc, single_assert))
  expect_silent(validate_consonance(abc, single_check))
  expect_error(capture_output(validate_consonance(1:3, single_test)))
  expect_error(capture_output(validate_consonance(1:3, single_assert)))
  expect_error(capture_output(validate_consonance(1:3, single_check)))
})

test_that("test_consonance with verbose output", {
  expect_silent(validate_consonance(abc, suite_assert))
  # message should include INFO field and name of test
  expect_output(validate_consonance(abc, suite_verbose), "INFO")
  expect_output(validate_consonance(abc, suite_verbose), "chars")
})
...
test_that("test_consonance with verbose output", {
  ...
  expect_silent(validate_consonance(c(2, 4, 6), suite))
  expect_error(capture_output(validate_consonance(c(2, 4, 5), suite)))
  expect_error(capture_output(validate_consonance(c(2, -4, 6), suite)))
})
...
test_that("test_consonance of data frames", {
  ...
  expect_silent(validate_consonance(m_square, suite))
  expect_error(capture_output(validate_consonance(m_long, suite)))
})
...
test_that("test_consonance of data frame columns", {
  ...
  expect_silent(validate_consonance(df, suite_a))
  expect_error(capture_output(validate_consonance(df, suite_b)))
})
...
test_that("signals level=warning and level=errors differently", {
  ...
  expect_error(capture_output(validate_consonance(-4, suite_e)))
  expect_error(capture_output(validate_consonance(-4, suite_e)))
  # suite w should have warnings
  expect_output(validate_consonance(-4, suite_w), "WARN")
  expect_output(validate_consonance(-4, suite_w), "warning")
  # but suite w should not generate errors
  expect_failure(
    expect_output(validate_consonance(-4, suite_w), "ERROR")
  )
})
...
test_that("signals R warning() as WARN", {
  ...
  expect_silent(validate_consonance(good_df, suite_spearman))
  # column in bad_df have ties, which should trigger an R warning
  expect_output(validate_consonance(bad_df, suite_spearman), "WARN")
})
...
test_that("suite constructor logs into a file", {
  ...
  expect_error(validate_consonance("abc", suite_file))
...
test_that("test_consonance can log into a file on request", {
  ...
  expect_error(validate_consonance(1:3, suite_assert, log.file=file_path))
  expect_true(file.exists(file_path))
  expect_equal(length(readLines(file_path)), 2)
  # a second run without log.file should not generate additional lines
  expect_error(capture_output(validate_consonance(1:3, suite_assert)))
  expect_equal(length(readLines(file_path)), 2)
...
test_that("test_consonance can skip all tests", {
  ...
  expect_silent(validate_consonance(1:3, suite_assert, skip=TRUE))
  # when skip is TRUE, the output should still be equal to the input
  result <- validate_consonance(1:3, suite_assert, skip=TRUE)
  expect_equal(result, 1:3)
})

test_that("skipping tests logs the skip", {
  # normal execution should report test results
  expect_output(validate_consonance(abc, suite_verbose), "pass")
  # skip should record skip action
  expect_output(validate_consonance(abc, suite_verbose,
                                    skip=TRUE, skip.action = "log"),
                "skip")
  # skipping should be be able to be entirely quiet
  expect_silent(validate_consonance(abc, suite_verbose,
...
test_that("test_consonance can use different logging level", {
  ...
  expect_output(validate_consonance(abc, suite_assert, logging.level="INFO"))
})

test_that("test_consonance can avoid logging altogether at run-time", {
  s <- suite_assert
  # no output on good data
  expect_silent(validate_consonance(abc, s, logging.level=NULL))
  # error and output on bad data
  expect_output(expect_error(validate_consonance(1:3, s)))
  # error only but not output
  expect_silent(expect_error(validate_consonance(1:3, s, logging.level=NULL)))
})
...
test_that("test_consonance can toggle strictness at runtime", {
  ...
  expect_output(validate_consonance(1:3, suite_warning), "WARN")
  # but can halt if runtime level is more strict
  expect_error(capture_output(
    validate_consonance(1:3, suite_assert, level="warning")))
})
