...
custom_function_1 <- function(x, skip.qc=FALSE) {
  test_consonance(x, suite_arguments, skip=skip.qc)
...
large_data %>%
  test_consonance(suite_pipeline) %>%
  work_on_data_1 %>%
  test_consonance(suite_pipeline) %>%
  work_on_data_2
# edits to pseudo-code for debugging
large_data %>%
  test_consonance(suite_pipeline, skip=TRUE) %>%
  work_on_data_1 %>%
  test_consonance(suite_pipeline, logging.level="INFO") %>%
  work_on_data_2
```
...
load("model.Rda")
new_data %>% test_consonance(model) %>% predict(object=model)
```
