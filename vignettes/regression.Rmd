...
d_train %>% test_consonance(model_two_A)
d_test %>% test_consonance(model_twol_A)
# the corrupted datasets will generate messages
d_corrupt_3 %>% test_consonance(model_two_A)
d_corrupt_4 %>% test_consonance(model_two_A)
```
...
As before, we can evaluate consonance between our original and our corrupted
datasets with these new models.

```{r, error=TRUE}
# the training and test datasets should evaluate quietly
d_train %>% test_consonance(model_all_B)
d_test %>% test_consonance(model_all_B)
# the corrupted datasets will generate messages
d_corrupt_3 %>% test_consonance(model_all_B)
d_corrupt_4 %>% test_consonance(model_all_B)
```
